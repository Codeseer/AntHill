// Generated by CoffeeScript 1.4.0
(function() {
  var Project;

  Project = require('../models/project').Model;

  module.exports = function(app) {
    app.get('/projects/new', function(req, res) {
      return res.render('project/new');
    });
    app.post('/projects', function(req, res) {
      var b, newProject;
      if (!req.user) {
        res.flash('error', 'You must be logged in to create a new project.');
        return res.redirect('/login');
      }
      b = req.body;
      if (b.projectname) {
        newProject = new Project();
        newProject.name = b.projectname;
        newProject.users.push({
          username: req.user.username,
          role: 'manager'
        });
        return newProject.save(function(err) {
          if (err) {
            res.flash('error', err + '');
            return res.redirect('/');
          } else {
            res.flash('success', 'Your project has been created.');
            return res.redirect('/');
          }
        });
      } else {
        res.flash('error', 'Please provide project with a name.');
        return res.redirect('/');
      }
    });
    app.get('/projects/:name', function(req, res) {
      return Project.findOne({
        name: req.params.name
      }).exec(function(err, project) {
        if (project) {
          return res.render('project/index', {
            'project': project
          });
        } else {
          return res.redirect('/404');
        }
      });
    });
    app.put('/projects/:name', function(req, res) {});
    app["delete"]('/projects', function(req, res) {
      req.user.isManager(req.body.name, function(manager) {
        if (!manager) {
          return res.redirect('/projects');
        }
      });
      return Project.findOne({
        name: req.body.name
      }).remove(function(err) {
        return res.redirect('/projects');
      });
    });
    app.post('/projects/:name/users', function(req, res) {
      return req.user.isManager(req.params.name, function(manager) {
        if (!manager) {
          res.flash('error', 'You are not the manager');
          return res.redirect('/projects/' + req.params.name);
        }
        return Project.findOne({
          name: req.params.name
        }).exec(function(err, project) {
          project.users.push({
            username: req.body.username,
            role: req.body.role
          });
          return project.save(function(err) {
            console.log(err);
            console.log(project);
            return res.redirect('/projects/' + req.params.name);
          });
        });
      });
    });
    app.put('/projects/:name/users/:username', function(req, res) {
      return req.user.isManager(req.params.name, function(manager) {
        var username;
        if (!manager) {
          res.flash('error', 'You are not the manager');
          return res.redirect('/projects/' + req.params.name);
        }
        username = req.params.username;
        return Project.findOne({
          name: req.params.name
        }).exec(function(err, project) {
          var count, index, user, _i, _len, _ref;
          index = -1;
          count = 0;
          _ref = project.users;
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            user = _ref[_i];
            if (user.username === username) {
              index = count;
            }
            count++;
          }
          if (index !== -1) {
            project.users[index].role = req.body.role;
            return project.save(function(err) {
              return res.redirect('/projects/' + req.params.name);
            });
          } else {
            return res.redirect('/projects/' + req.params.name);
          }
        });
      });
    });
    return app["delete"]('/projects/:name/users', function(req, res) {
      var username;
      req.user.isManager(req.params.name, function(manager) {
        if (!manager) {
          res.flash('error', 'You are not the manager');
          return res.redirect('/projects/' + req.params.name);
        }
      });
      if (req.user.username === req.body.username) {
        res.flash('error', 'You can not remove yourself.');
        return res.redirect('/projects/' + req.params.name);
      }
      username = req.body.username;
      return Project.findOne({
        name: req.params.name
      }).exec(function(err, project) {
        var count, index, user, _i, _len, _ref;
        index = -1;
        count = 0;
        _ref = project.users;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          user = _ref[_i];
          if (user.username === username) {
            index = count;
          }
          count++;
        }
        if (index !== -1) {
          project.users[index].remove();
          return project.save(function(err) {
            return res.redirect('/projects/' + req.params.name);
          });
        } else {
          return res.redirect('/projects/' + req.params.name);
        }
      });
    });
  };

}).call(this);
